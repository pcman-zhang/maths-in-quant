...
(a) 标准差法    sqrt(Var(r))*sqrt(252): 年化波动率 = 22.81%
(b) 二次变差法  sqrt(sum(r^2)/年数)   : 年化波动率 = 22.81%
(两种方法几乎一致 —— sum(r^2) 就是总波动的载体, 这正是 (dB)^2=dt 的实证版本)

累积已实现方差 vs 时间正比线:
    时间占比 |     实际 cumRV |        正比线预测 |      比值
     0.25 |           0.0328 |           0.0373 |       0.879
     0.50 |           0.0940 |           0.0746 |       1.261
     0.75 |           0.1188 |           0.1119 |       1.061
     1.00 |           0.1492 |           0.1492 |       1.000

但局部看, 波动率并不均匀: 滚动60日已实现波动率区间 12.5% ~ 43.5%
```

![二次变差的实证: 左图-茅台累积已实现方差沿时间正比线上下波动地爬升; 右图-滚动60日已实现波动率在12.5%到43.5%之间聚集波动](images/ch24_fig4_realized_variance.png)

**三层解读**:

1. **全局成立**: 两种完全不同的估计器给出同一个年化波动率 22.81%——$\sum r^2$ 确实承载了几乎全部波动信息。波动率交易中的核心品种"已实现方差" (Realized Variance) 就建立在这个量上。
