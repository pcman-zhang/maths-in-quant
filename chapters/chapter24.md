...
要证明链式法则在随机世界失效, 最简洁的方式是找一个**能精确算出正确答案的非线性函数**, 然后看普通链式法则给出的答案偏差多大。$x^2$ 是最好的候选: 它足够简单 (泰勒展开只有两项), 又有确定的解析解 ($E[B_T^2] = T$, 第23章的方差公式)。如果链式法则在这么简单的函数上都会出错, 更复杂的函数只会更糟。
...
**它解决什么问题**: 上一节的实验告诉我们链式法则在随机世界会出错, 但没有告诉我们怎么修。伊藤引理就是修复后的链式法则——它在普通微积分的基础上多加了一个二阶修正项。
