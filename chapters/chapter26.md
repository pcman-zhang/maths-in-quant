...
print("=== 工作流演示: 用第24章估计的真实参数给衍生品定价 ===")
sigma_hat = 0.2281          # 第24章: 贵州茅台日收益率 -> 年化波动率 22.81%
...
```
=== 工作流演示: 用第24章估计的真实参数给衍生品定价 ===
输入: sigma=22.81% (来自历史数据), r=2% (假设), mu 不需要!
(第25章的教训: 风险中性定价只用得到波动率)

结构 (K=S=100, T=1年)         |     MC价格 |     标准误
--------------------------------------------------
欧式 Call                    |   9.9802 |  0.0501
算术平均亚式 Call                |   5.6885 |  0.0277
浮动行权回看 Call                |  17.0607 |  0.0508
向下敲入 Put (B=85)            |   7.4423 |  0.0354
```
...
**题2——完整生产线的敏感性分析**: 参考 26.5 节的工作流, 固定 $\sigma = 22.81\%$、$r = 2\%$, 对 $K \in \{90, 100, 110\} \times T \in \{0.5, 1.0\}$ 的六种组合定价算术平均亚式 Call (建议叠加对偶+控制变量)。整理成 $3 \times 2$ 价格矩阵, 并回答: 行权价每上移 10 元, 亚式期权掉价多少? 期限缩短一半呢? 你的表格符合"期权价值随实值程度和期限递增"的一般规律吗?
