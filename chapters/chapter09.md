...
**金融直觉 2——股票的期望日收益率**: 以贵州茅台为例, 从 `ifind_price_data.csv` 中提取 723 个交易日的对数收益率, 取平均得到日均对数收益约 $-0.0245\%$. 这不是说茅台每天都跌 $0.0382\%$——它有时候涨 5%, 有时候跌 4%. 但如果你持有茅台 3 年, 你的**累积收益**取决于这些日收益的**总和**, 而日均收益给出了一个粗略的方向性指引.
...
- 无条件期望 $E[X] = -0.0245\%$/天——在不考虑任何信息时, 你对茅台收益率的最佳预测.
...
贵州茅台           -0.0245%     -6.19%    1.4319%     22.73%
宁德时代            0.0933%     23.52%    2.4313%     38.60%

年化协方差矩阵:
          沪深300      贵州茅台      宁德时代
沪深300  0.029936  0.023818  0.045611
贵州茅台   0.023818  0.051666  0.038460
宁德时代   0.045611  0.038460  0.148958

相关系数矩阵:
        沪深300    贵州茅台    宁德时代
沪深300  1.0000  0.6056  0.6830
贵州茅台   0.6056  1.0000  0.4384
宁德时代   0.6830  0.4384  1.0000
```

**三个关键观察**:

1. **风险与收益的经典权衡**: 宁德时代年化收益最高(23.52%), 但波动率也最高(38.60%). 沪深 300 最"稳"(17.30%), 但收益也最低(8.01%). 贵州茅台在这个样本期是一个反例——高波动(22.73%)却负收益(-6.19%), 提醒我们**波动率是风险度量, 不是方向性指标**.
...
- **对角线(左上→右下)**: 是各资产自身的方差 $\sigma_i^2$. 宁德时代的对角线格颜色最深(0.1490), 对应它最高的年化波动率(38.65%). 沪深 300 的对角线格颜色最浅(0.0299), 对应最低波动率(17.30%).
...
两资产组合分析:
  ...
  贵州茅台: 年化收益 = -6.19%, 年化波动 = 22.73%
  相关系数: 0.6056
  最小方差组合: 沪深300 权重 = 81.9%, 收益 = 5.44%, 波动 = 16.98%
  与单独持有贵州茅台相比: 收益变化 = +11.63%, 波动变化 = -5.75%
```

**关键发现**:

1. 尽管贵州茅台在样本期的年化收益为 **负**(-6.19%), 但将它以约 10% 的权重加入组合时, 组合波动率实际上低于单独持有沪深 300. 这就是"不完美相关($\rho < 1$)资产的组合可以降低风险"的数学体现——即使其中一只资产的单独表现很糟糕.
...
真实数据分散化效果:
  单资产平均波动率: 26.21%
  两资产等权平均波动率: 23.43% (降低 11%)
  三资产等权波动率: 22.27% (降低 15%)

理论极限(ρ=0.5): 波动率 = 14.14%
```
