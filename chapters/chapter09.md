# 第9章 期望、方差与协方差——风险与收益的度量

> **阶段定位**: 第二阶段「概率统计——量化的核心语言」
> **学习目标**: 从单资产的分布特征迈向多资产的"风险-收益"权衡, 建立"分散化降低风险"的数学直觉, 为 Markowitz 投资组合理论奠定基础.
...
本章将从期望出发, 经过方差和协方差, 最终抵达组合风险的完整表达式 $w^T \Sigma w$. 如果你觉得这个公式眼熟——第 3 章的导数告诉你"单个资产的敏感度", 第 4 章的积分教你"把无穷多微小量加起来". 本章把这两者合在一起: **协方差矩阵 $\Sigma$ 是多资产的"敏感度", 权重向量 $w$ 决定了每个资产贡献多少——$w^T \Sigma w$ 就是把所有资产对组合风险的贡献"积分"起来.**
...
**一个关键问题: 用 3 年的日收益率估计年化期望收益, 有多准?**

以沪深 300 为例: 日对数收益率的均值 $\hat{\mu}_{\text{daily}} = 0.0318\%$, 年化 $\hat{\mu}_{\text{annual}} = 0.0318\% \times 252 = 8.01\%$. 但这个 8.01% 有多可靠?

日收益率的标准差约为 $1.09\%$, 样本量 $n = 723$. 样本均值的标准误(Standard Error)为:

$$\text{SE}(\hat{\mu}_{\text{daily}}) = \frac{\sigma_{\text{daily}}}{\sqrt{n}} = \frac{0.0109}{\sqrt{723}} \approx 0.00041 = 0.041\%$$

年化后: $\text{SE}(\hat{\mu}_{\text{annual}}) = 0.041\% \times 252 \approx 10.2\%$.

这意味着沪深 300 的"真实"年化期望收益, 在 95% 置信水平下大约在 $8.01\% \pm 2 \times 10.2\% \approx [-12.4\%, 28.4\%]$ 之间. 范围宽得吓人——三年的数据远远不足以精确估计期望收益. **这就是为什么期望收益的估计是量化中最困难的问题之一**, 也是为什么 Markowitz 优化对输入参数(尤其是期望收益)极其敏感.
...
print(f"年化期望收益: {mu_daily*252*100:.2f}%")
print(f"年化期望收益的 95% 置信区间: [{mu_daily*252*100 - 2*se_daily*252:.1f}%, "
      f"{mu_daily*252*100 + 2*se_daily*252:.1f}%]")
print(f"\n结论: 即使有 3 年数据, 年化期望收益的估计仍然非常不精确.")
print(f"      这是所有均值-方差优化面临的'垃圾进, 垃圾出'问题——")
print(f"      期望收益的小估计误差, 会导致最优权重的巨大变化.")
```
...
**协方差矩阵的估计是实现组合优化的最大挑战**. 当 $n=500$ 时, 你需要估计 $500 \times 501 / 2 = 125,250$ 个参数——但如果你只有 3 年的日数据(约 750 个交易日), 那么"样本量 < 参数量的 1%". 直接用样本协方差矩阵做优化几乎必然导致极端权重和糟糕的样本外表现. 这就是为什么业界使用**收缩估计(Shrinkage)**, **因子模型**, 和**贝叶斯先验**来改善协方差矩阵的估计——这些是第 14 章和第 17 章的主题.
...
#### 补充: 协方差矩阵的收缩估计——从"纸面"到"实盘"

在 9.4.4 节我们提到: 用 723 个交易日估计 $3 \times 4 / 2 = 6$ 个协方差参数, 样本量远大于参数量, 暂时安全. 但当 $n=100$ 时, 你需要估计 5050 个参数——而你可能只有 500 个交易日的数据. 直接用样本协方差矩阵做优化, 几乎必然导致**极端权重**和**糟糕的样本外表现**.
...
其中 $\delta \in [0,1]$ 是收缩强度(由数据自动选择), $\Sigma_{\text{structured}}$ 是一个高度结构化的"目标矩阵"(如所有相关系数设为平均相关). 直觉: 样本协方差可能过拟合, 结构化目标可能过于简化——取两者的加权平均, 用数据决定权重.
...
**收缩估计与贝叶斯的联系**: 收缩估计本质上就是贝叶斯——$\Sigma_{\text{structured}}$ 是先验, $\Sigma_{\text{sample}}$ 是数据似然, $\Sigma_{\text{shrunk}}$ 是后验. $\delta$ 由数据自动选择, 等价于用经验贝叶斯方法估计了先验的强度. 这是第 7 章贝叶斯公式在协方差矩阵估计中的精彩应用.

---

### 9.8.2 可视化: 协方差热力图与相关性情雨图
...
### 9.9.1 总风险 = 系统性风险 + 特有风险

$$\sigma_p^2 = \underbrace{\rho\sigma^2}_{\text{系统性风险}} + \underbrace{\frac{1-\rho}{n}\sigma^2}_{\text{可分散风险}}$$

- **系统性风险(Systematic Risk)**: 由宏观经济, 政策, 市场情绪等共同因素驱动, **无法通过分散化消除**. 在 CAPM 中, 它由 $\beta$ 度量.
- **特有风险(Idiosyncratic Risk)**: 由个股特有因素(管理层变动, 产品发布, 诉讼)驱动, **可通过持有 20-30 只不相关资产基本消除**.

### 9.9.2 为什么分散化有效?

两个不完全相关的资产组合在一起, 它们的波动在时间上**部分抵消**: 当 A 跌 2% 时, B 可能只跌 0.5% 甚至涨 1%. 只要 $\rho < 1$, 组合的方差就小于各资产方差的加权平均——这是协方差项 $2w(1-w)\rho\sigma_A\sigma_B$ 在起作用.

### 9.9.3 分散化的边界与危机中的失效

- $\rho = 0$(理想情况): 组合方差 $\propto 1/n$, 随着 $n$ 增大趋近于 0. 现实中几乎不存在.
- $\rho = 1$(最坏情况): 分散化完全无效, 组合风险等于单资产风险.
- **真实市场**: $\rho \approx 0.3 \sim 0.7$, 分散化可降低 40-60% 的波动率.
- **2008 年危机**: 所有风险资产的 $\rho \to 1$, 分散化临时失效. 平时看似不相关的资产(股票, 高收益债, REITs)全部同步暴跌. 这一现象称为"相关性崩溃(Correlation Breakdown)"——它提醒我们, 历史相关系数是**有条件的**, 在尾部事件中可能完全不可靠.

### 9.9.4 跟踪误差: 绝对风险 vs 相对风险
...
假设你的基准是沪深 300(年化波动 17.3%). 你的组合年化波动 15%——看起来不错, 风险比基准低. 但如果跟踪误差高达 8%, 这说明你的组合和基准的构成差异很大——你承担了大量的**主动风险**. 在牛市中你可能大幅跑赢基准, 但在熊市中你同样可能大幅跑输.

**信息比率(Information Ratio, IR)** 是机构考核 PM 的核心 KPI:

$$\text{IR} = \frac{E[R_p - R_b]}{\text{TE}} = \frac{\text{超额收益}}{\text{跟踪误差}}$$

IR 衡量的是"每承担一单位主动风险, 获得了多少超额收益". IR > 0.5 通常被认为是优秀的主动管理能力. Grinold & Kahn(1999)的"基本主动管理定律"指出:

$$\text{IR} \approx \text{IC} \times \sqrt{\text{Breadth}}$$

其中 IC 是信息系数(预测与实现的相关系数), Breadth 是独立预测的次数. 这个公式将"预测能力"和"策略容量"连接了起来——我们将在第 12 章回归分析中重新审视它.
...
| 要点 | 核心内容 |
|------|----------|
| **期望** | 长期平均收益, 具有线性性质——组合收益 = 各资产收益的加权平均 |
| **方差** | 风险的数学定义, 对称惩罚涨跌——业界用半方差和 CVaR 补充 |
| **协方差** | 捕捉资产间的线性协同运动, 是分散化能否生效的关键 |
| **相关系数** | 标准化到 [-1,1], 但只度量线性关系, 忽视尾部依赖 |
| **协方差矩阵** | $n(n+1)/2$ 个参数——估计它是组合优化的最大挑战 |
| **组合方差** | $\sigma_p^2 = w^T \Sigma w$——Markowitz 优化的核心表达式 |
| **分散化** | 特质风险 $\propto 1/n$, 系统性风险是下限, 相关性决定分散化效果 |
| **2008 教训** | 危机中 $\rho \to 1$, 分散化临时失效——历史相关系数不可盲信 |

> **量化箴言**: "不要把所有鸡蛋放在一个篮子里"——但更重要的是, **篮子之间不能高度相关**. 当所有篮子同时着火时(2008), 分散化毫无意义. 量化风控的核心任务, 就是不断追问: **当前的 $\Sigma$ 在极端市场中还可靠吗?**
...
3. **Grinold, R. C., & Kahn, R. N. (1999).** *Active Portfolio Management* (2nd ed.). McGraw-Hill. —— 第 2-3 章从信息比率角度重新诠释风险-收益权衡.
...
**题 4 — 协方差矩阵的估计窗口敏感性**: 基于 9.8 节的代码, 完成以下分析.
