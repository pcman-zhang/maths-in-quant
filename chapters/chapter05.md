...
![e的自然涌现: 复利频率从年(n=1)到日(n=365)到无限(n→∞), 终值收敛到e](images/ch05_fig1_e_emergence.png)
...
![对数收益率的时间可加性: 左图-逐期简单vs对数对比, 右图-对数之和精确等于log(P_t/P_0)](images/ch05_fig2_log_return_additivity.png)
...
- 对数收益率：$\ln(1.10) + \ln(0.90) \approx 0.0953 - 0.1054 = -0.0101$（同样为负）

注意：同幅度的"先涨后跌"，总收益本来就应该是负的——先涨 $10\%$ 再跌 $10\%$，期末价格只有期初的 $0.99$ 倍。对数收益率的真正对称性在于：**价格涨上去再跌回原点，两段对数收益率恰好互为相反数**：

$$
\ln\left(\frac{P_1}{P_0}\right) = -\ln\left(\frac{P_0}{P_1}\right)
$$

比如价格先涨 $10\%$（$r_1 \approx +0.0953$），再恰好跌回原价（$r_2 \approx -0.0953$），两段对数收益率精确抵消，总和为零。这种"涨跌镜像"的对称性让多期收益的数学处理更加干净，而简单收益率不具备这个性质。
...
- 当收益率达到 $+20\%$ 时，误差约 $0.018$；而 $-20\%$ 时误差约 $0.023$——注意近似在下跌一侧偏差更大（因为 $\ln(1+x)$ 只在 $x > -1$ 处有定义且左侧弯曲更厉害），此时需要考虑二阶修正项 $-x^2/2$。
...
# 注意：这里用了 mu - 0.5*sigma^2，这是伊藤引理修正项，后续随机分析章节会详细讲解
