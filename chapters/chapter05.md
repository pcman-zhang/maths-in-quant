...
**核心结论**：逐期对数收益率相加，精确等于用首尾价格直接算出的总对数收益率。而简单收益率的"和"不等于总收益，必须做"乘积减一"的复合运算。
...
**关键观察**：
- 当收益率在 $\pm 5\%$ 以内时，$\ln(1+x) \approx x$ 的误差小于 $0.001$（万分之一），几乎可以忽略；
- 当收益率达到 $\pm 20\%$ 时，误差扩大到约 $0.02$（2%），此时需要考虑二阶修正项 $-x^2/2$。
