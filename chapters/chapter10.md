...
### 10.2.2 用真实数据验证: 沪深300 日收益率的均值收敛
...
**关键观察**: 与伯努利模拟不同, 真实收益率均值的收敛曲线在早期阶段可能受市场趋势影响而持续偏向一侧. 收敛带(粉色)在初期很宽, 随着 $n$ 增大逐渐收窄——这正是 $\sigma / \sqrt{n}$ 衰减的直接体现. 注意, 即使 $n=723$, 日均收益率的 95% 置信区间仍然相当宽.
...
### 10.4.3 实例: 沪深300 期望收益的估计精度

回到第 9 章的问题: 沪深 300 的日对数收益率均值 $\hat{\mu}_{\text{daily}} = 0.0318\%$, 年化 $\hat{\mu}_{\text{annual}} = 8.01\%$. 这个 8.01% 有多可靠?

日收益率的标准差约为 $1.09\%$, 样本量 $n = 723$. 样本均值的标准误为:
...
这意味着沪深 300 的"真实"年化期望收益, 在 95% 置信水平下大约在 $8.01\% \pm 1.96 \times 10.2\% \approx [-12.0\%, 28.0\%]$ 之间. **范围宽得吓人**——三年的数据远远不足以精确估计期望收益.
...
均值的标准误(日): 0.000405 (0.0405%)
年化期望收益(点估计): 8.01%
年化标准误: 10.21%
年化期望收益的 95% 置信区间: [-12.0%, 28.0%]

结论: 即使有 3 年数据, 年化期望收益的估计仍然非常不精确.
      这是所有均值-方差优化面临的'垃圾进, 垃圾出'问题——
      期望收益的小估计误差, 会导致最优权重的巨大变化.
```
...
1. **期望收益的精确估计几乎不可能**: 即使有 10 年的日数据($n \approx 2520$), 年化 SE 仍然有 $0.0109 / \sqrt{2520} \times 252 \approx 5.5\%$. 95% 置信区间仍然宽达 22 个百分点.

2. **量化中更注重"相对"而非"绝对"**: 与其争论某只股票的期望收益是 8% 还是 10%, 不如关注它相对于同类的**排序**是否稳定——排序对估计误差的敏感度远低于绝对值.
